...
# 函式：統一的敘事渲染節點 (v26.0 - 中心化格式化)
# 更新紀錄:
# v26.0 (2025-09-09): [災難性BUG修復] 根據 ai_core v216.0 的重構，此節點現在承擔了構建完整 system prompt 字符串的核心職責。它使用標準的 Python f-string 來格式化一個包含所有必要指令的模板，然後將其與 human prompt 一起傳遞給簡化後的渲染鏈。此修改將所有複雜的格式化操作集中在一個地方，從根本上解決了 LangChain 模板解析器因嵌套變數而引發的 ValueError。
# v25.0 (2025-09-09): [災難性BUG修復] 簡化了傳遞給渲染鏈的數據流以解決 KeyError。
# v24.0 (2025-09-06): [災難性BUG修復] 在此節點增加了“執行否決”檢查。
async def narrative_rendering_node(state: ConversationGraphState) -> Dict[str, str]:
    ...
    # [v26.0 核心修正] 在節點內部使用 f-string 構建完整的 system prompt
