# src/lore_tools.py 的中文註釋(v2.0 - 參數健壯性)
# 更新紀錄:
# v2.0 (2025-09-10): [災難性BUG修復] 為所有工具的Pydantic參數模型中的關鍵欄位增加了 `validation_alias=AliasChoices`。此修改極大地增強了工具對上游LLM生成的不規範參數名稱的容錯能力，是解決 ValidationError 的第一道防線。
...
class CreateNewNpcProfileArgs(BaseToolArgs):
    ...
    original_name: str = Field(
        description="LLM 在計畫中生成的原始 NPC 名稱。",
        validation_alias=AliasChoices('original_name', 'name', 'npc_name')
    )
...
class AddOrUpdateLocationInfoArgs(BaseToolArgs):
    ...
    original_name: str = Field(
        description="LLM 在計畫中生成的原始地點名稱。",
        validation_alias=AliasChoices('original_name', 'name', 'location_name')
    )
...
class AddOrUpdateItemInfoArgs(BaseToolArgs):
    ...
    original_name: str = Field(
        description="LLM 在計畫中生成的原始物品名稱。",
        validation_alias=AliasChoices('original_name', 'name', 'item_name')
    )
...
class DefineCreatureTypeArgs(BaseToolArgs):
    ...
    original_name: str = Field(
        description="LLM 在計畫中生成的原始生物名稱。",
        validation_alias=AliasChoices('original_name', 'name', 'creature_name')
    )
...
class AddOrUpdateQuestLoreArgs(BaseToolArgs):
    ...
    original_name: str = Field(
        description="LLM 在計畫中生成的原始任務標題。",
        validation_alias=AliasChoices('original_name', 'name', 'title', 'quest_name')
    )
    description: str = Field(
        description="任務的詳細描述，包括背景、目標、獎勵等。",
        validation_alias=AliasChoices('description', 'content', 'quest_description')
    )
    location_path: List[str] = Field(description="觸發或與該任務相關的地點路径。")
...
class AddOrUpdateWorldLoreArgs(BaseToolArgs):
    ...
    original_name: str = Field(
        description="LLM 在計畫中生成的原始傳說標題。",
        validation_alias=AliasChoices('original_name', 'name', 'title', 'lore_name')
    )
    content: str = Field(
...
async def add_or_update_world_lore(lore_key: str, standardized_name: str, original_name: str, content: str) -> str:
    ...
    return f"已成功將 '{standardized_name}' 記錄為世界傳說。"
