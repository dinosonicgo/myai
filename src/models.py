...
# models.py 的 GameState 模型 (v18.0 - 對話焦點記憶)
# 更新紀錄:
# v18.0 (2025-12-18): [架構擴展] 新增了 `last_explicit_speaker` 欄位。此欄位用於記錄上一輪對話的明確發言者，作為「雙層對話焦點系統」的程式碼備援層，以解決在對話焦點模糊時的判斷問題。
# v17.2 (2025-10-08): [架構擴展] 新增了 `is_in_genesis_phase` 旗标，用于实现“新手保护期”状态机。
...
class GameState(BaseModel):
    ...
    # [v18.0 新增] 對話焦點記憶
    last_explicit_speaker: Optional[str] = Field(default=None, description="上一輪對話中，被程式碼解析出的、明確的發言者。用於對話焦點的程式碼備援。")
