...
# main.py 的新函式
# 函式：驗證資料庫結構並在必要時重建 (v1.0 - 全新創建)
# 更新紀錄:
# v1.0 (2025-12-10): [全新創建] 為了處理因模型變更導致的資料庫結構不匹配問題，創建此函式。它會在啟動時自動檢測 `lore_book` 表是否包含新結構的欄位，如果沒有，則會備份並刪除舊的資料庫檔案，強制系統在下次啟動時創建一個全新的、結構正確的資料庫。
def _verify_and_rebuild_database():
    """
    在非同步環境啟動前，同步檢查資料庫結構是否過時。
    如果結構過時，則備份並刪除舊資料庫，以便 SQLAlchemy 可以重新創建。
    """
    import sqlite3
    ...
    if not db_path.exists():
        print("ℹ️ [資料庫檢查] 資料庫檔案不存在，將在啟動時自動創建。")
        return

    try:
        print("ℹ️ [資料庫檢查] 正在驗證資料庫結構...")
        con = sqlite3.connect(db_path)
        cursor = con.cursor()
        # 探測性查詢：嘗試查詢新模型中的關鍵欄位
        cursor.execute("SELECT structured_content, narrative_content FROM lore_book LIMIT 1;")
        cursor.fetchone()
        con.close()
        print("✅ [資料庫檢查] 資料庫結構驗證通過，無需重建。")
    except sqlite3.OperationalError as e:
        if "no such column" in str(e):
            print("⚠️ [資料庫檢查] 檢測到過時的資料庫結構！")
            print(f"   錯誤詳情: {e}")
            ...
            try:
                ...
                print(f"   - 已將舊資料庫備份至: {backup_path}")
            except Exception as backup_err:
                print(f"   🔥 備份舊資料庫失敗: {backup_err}")
            ...
            try:
                ...
                os.remove(db_path)
                print("   - 舊的資料庫檔案已被移除。")
                print("   - 系統將在下次啟動時自動創建一個全新的、結構正確的資料庫。")
            except Exception as del_err:
                print(f"   🔥 自動刪除舊資料庫失敗: {del_err}")
                print("   - 請手動刪除以下檔案後重啟程式：")
...
# main.py 的 main 函式 (v14.0 - 資料庫自動重建)
# 更新紀錄:
# v14.0 (2025-12-10): [災難性BUG修復] 在程式啟動的最開始，增加了對 `_verify_and_rebuild_database()` 的調用。此機制會自動檢測資料庫結構是否過時，並在必要時備份和刪除舊資料庫，強制系統重新創建，從而根除因模型變更導致的 `no such column` 致命錯誤。
# v13.0 (2025-11-26): [重大架構重構] 徹底移除了在 `main.py` 中直接調用 `_check_and_install_dependencies` 的邏輯。
# v12.0 (2025-11-26): [重大架構升級] 在程式啟動的最開始，增加了對 `_setup_huggingface_mirror()` 和 `_check_and_install_dependencies()` 的調用。
async def main():
    MAIN_PY_VERSION = "v14.0" # 版本號更新
    print(f"--- AI Lover 主程式 ({MAIN_PY_VERSION}) ---")
    
    # [v14.0 核心修正] 在所有操作之前，首先驗證資料庫結構
    _verify_and_rebuild_database()
